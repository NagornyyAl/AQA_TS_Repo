...
Тому Cypress-тести для Rozetka виконуються на локальній HTML-фікстурі, яка повторює потрібні елементи, селектори та дії з домашнього завдання №15. Це дозволяє перевірити логіку автотестів: введення тексту, кліки, перехід у категорію, роботу з кошиком та перевірки.

Puppeteer-тести для Rozetka відкривають реальний сайт напряму. Якщо сайт повертає Cloudflare challenge, локальний запуск може падати через зовнішній захист, а не через помилку в тестовій логіці.
