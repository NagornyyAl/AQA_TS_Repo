...
3. `GET /images/search?breed_ids={breed_id}&has_breeds=true&limit=1`
    ...
    - Перевірити, що `image.breeds[]` містить вибраний `breed.id`.
4. `POST /votes`
...
1. `Cat API Integration Flow`
    ...
    - Сценарій створює унікальний `sub_id`, отримує породу, знаходить зображення, створює vote і favourite, перевіряє їх через list/detail endpoints і в кінці видаляє створені ресурси.
